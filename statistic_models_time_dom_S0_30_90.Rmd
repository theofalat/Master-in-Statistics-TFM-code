...
read_h5_mic_file <- function(filename="S0_30_90.h5", downsample_factor=10){
...
mic_color_map <- data.frame(
  mic_id = seq_len(nrow(position_cartesian)),  
  color = mic_colors
)
...
# Run t-SNE
tsne_results <- Rtsne(mic_time_data[-50, ], perplexity = 2, initial_dims = 13, theta = 0.1, eta = 10, dims = 2, pca = TRUE, verbose = TRUE)
...
# ======= USER INPUTS =======
selected_mics <- c(8, 6, 7, 23, 22, 24, 42, 45, 3, 5, 30, 20, 4)
reference_mic <- 8  # Chosen reference mic
...
reference_mic <- 8              # Microphone to use as reference
fixed_lag <- 30                 # Lag to extract correlation at
...
# ======= USER INPUTS =======
selected_mics <- c(1:56)  # Mics to include
reference_mic <- 8
fixed_lag <- 100
sound_speed <- 340  # Optional, for expected delay

# ======= DATA PREP =======
all_mics <- unique(c(selected_mics, reference_mic))
all_mics <- setdiff(all_mics, 50)
mic_time_data_subset <- mic_time_data[all_mics, ]
mic_coords <- position_cartesian[all_mics, ]
mic_coords$MicID <- as.integer(rownames(mic_coords))

# Compute distances from reference mic
ref_coords <- as.numeric(position_cartesian[as.character(reference_mic), ])
mic_coords$Distance <- apply(mic_coords[, 1:3], 1, function(row) {
  sqrt(sum((as.numeric(row) - ref_coords)^2))
})
...
# ======= COMPUTE CORRELATIONS W.R.T REFERENCE =======
correlations <- sapply(all_mics, function(mic) {
  if (mic == reference_mic) return(1)
  get_ccf_at_fixed_lag(
    mic_time_data[reference_mic, ],
    mic_time_data[mic, ],
    lag = fixed_lag
  )
})

mic_coords$Correlation <- round(correlations, 3)
...
p3d <- plot_ly(
  ...
  text = ~paste("Mic", MicID, "<br>Corr =", Correlation),
  hoverinfo = "text"
) %>%
  layout(
    scene = list(
    ...
    title = paste("3D Mic Correlation Map (Lag =", fixed_lag, ")")
  )

p3d

# ======= SIDE PLOT: CORRELATION vs DISTANCE =======
mic_coords_filtered <- mic_coords %>% filter(MicID != reference_mic)

ggplot(mic_coords_filtered, aes(x = Distance, y = Correlation)) +
  geom_point(size = 3, color = "black", fill = "skyblue", shape = 21) +
  geom_text(aes(label = MicID), vjust = -1, size = 3.5) +  # Add mic labels
  geom_smooth(method = "loess", se = FALSE, color = "blue", linetype = "dashed") +
  labs(
    title = paste("Correlation vs Distance from Mic", reference_mic),
    subtitle = paste("Fixed Lag =", fixed_lag),
    x = "Distance (m)",
    y = "Cross-Correlation"
  ) +
  theme_minimal()
...
# Compute distances from the reference microphone
ref_coords <- mic_coords[mic_coords$MicID == reference_mic, c("x", "y", "z")]

# Compute mean coherence for each microphone
mic_coords$Coherence <- sapply(mic_coords$MicID, function(mic_id) {
  if (mic_id == reference_mic) return(1)
  result <- coh(
    mic_time_data[reference_mic, ],
    mic_time_data[mic_id, ],
    f = fs,
    plot = FALSE
  )
...
p3d <- plot_ly(
  ...
  marker = list(
    ...
    colorscale = "RdBu",
    cmin = 0.2, cmax = 0.7,
    colorbar = list(title = "Coherence")
  ),
  text = ~paste("Mic", MicID, "<br>Corr =", Coherence),
  hoverinfo = "text"
) %>%
  layout(
    scene = list(
    ...
    title = paste("3D Mic Coherence Map (Lag =", fixed_lag, ")")
  )

p3d

ggplot(mic_coords[mic_coords$MicID != reference_mic, ], aes(x = Distance, y = Coherence)) +
  geom_point(size = 3, color = "black", fill = "lightgreen", shape = 21) +
  geom_text(aes(label = MicID), vjust = -1, size = 3.5) +
  geom_smooth(method = "loess", se = FALSE, color = "darkgreen", linetype = "dashed") +
  labs(
    title = paste("Mean Coherence vs Distance from Mic", reference_mic),
    x = "Distance (m)",
    y = "Mean Coherence"
  ) +
  theme_minimal()
...
# Step 1: Run auto.gdpc on transposed data (gdpc expects time in rows)
gdpc_model <- auto.gdpc(multivar_data, crit="LOO", expl_var = 0.99)  # auto selects k and number of components
...
# Step 2: Extract dynamic components (scores)
# Extract all components' f vectors and combine
f_list <- lapply(seq_along(gdpc_model), function(i) {
  data.frame(Time = 1:length(gdpc_model[[i]]$f),
             Value = gdpc_model[[i]]$f,
             Component = paste0("DPC", i))
})

f_list <- f_list[1]

# Combine into one dataframe
f_long <- do.call(rbind, f_list)

# Plot
ggplot(f_long, aes(x = Time, y = Value, color = Component)) +
  geom_line(linewidth = 0.8) +
  labs(
    title = "Dynamic Principal Components from auto.gdpc()",
    x = "Time",
    y = "Component Value"
  ) +
  theme_minimal()
...
```{#r}
beta_matrix <- gdpc_model[[1]]$beta  # For DPC1
image(beta_matrix, main = "Beta Coefficients for DPC1")
```
```{#r}
# Combine beta magnitudes across components
beta_stack <- do.call(cbind, lapply(gdpc_model, function(m) {
  apply(abs(m$beta), 2, mean)
}))
rownames(beta_stack) <- paste0("Lag", 1:nrow(beta_stack))

heatmap(beta_stack, main = "Mic Contributions to Dynamic Components")
```
...
mic_time_data_new <- mic_time_data[-50, ]

# Perform ICA
ica_result <- fastICA(t(mic_time_data_new), n.comp = 3) 

```
ICA Components
```{r, fig.height=10, fig.width=10}
par(mfrow = c(3, 1))  # Adjust for how many components to view
for (i in 1:3) {
...
```{r}
library(pheatmap)
# Assign microphone numbers to columns:
colnames(ica_result$A) <- rownames(position_cartesian[-50, ])

# Assign ICA component numbers to rows:
rownames(ica_result$A) <- paste0("IC", 1:nrow(ica_result$A))

# Plot
pheatmap(ica_result$A,
         main = "Mixing Matrix: ICA Components x Microphones",
         cluster_rows = TRUE,
         cluster_cols = TRUE)
...
# Choose an ICA component to visualize (e.g., the 2nd)
component_index <- 1
loadings <- abs(ica_result$A[component_index, ])

loadings <- (loadings - min(loadings)) / (max(loadings) - min(loadings))

# Filter mic positions (remove mic 50 to match ICA input)
position_cartesian_filtered <- position_cartesian[-50, ]

# Plot with fixed color scale [0, 0.1]
plot_ly(
  ...
  marker = list(
    ...
    cmin = 0,
    cmax = 1,
    colorbar = list(title = "Absolute ICA Loading (0–1)")
  ),
  text = ~paste(rownames(position_cartesian_filtered)),
  hoverinfo = "text"
) %>%
  layout(
    scene = list(
...
# Clustering based on loadings
```{r}
# Load necessary libraries
library(cluster)
library(plotly)

# ==== USER SELECTION: Specify which IC components to visualize ====
selected_components <- c(1, 3)  

# ==== Setup ====
num_clusters <- 3  # Number of clusters for PAM
strength_colors <- c("Strong" = "red", "Medium" = "green", "Weak" = "blue")

# Ensure microphone IDs match
colnames(ica_result$A) <- rownames(position_cartesian_filtered)

# ==== Loop over selected ICA components ====
for (component_index in selected_components) {
  
  # 1. Extract loading vector from ICA matrix
  component_vector <- ica_result$A[component_index, ]
  component_vector <- as.numeric(component_vector)
  names(component_vector) <- colnames(ica_result$A)

  # 2. Scale (optional)
  component_vector_scaled <- scale(component_vector)

  # 3. Apply PAM clustering
  pam_result <- pam(component_vector_scaled, k = num_clusters)
  position_cartesian_filtered$Cluster <- as.factor(pam_result$clustering)

  # 4. Rename clusters for interpretation
  cluster_means <- tapply(component_vector, pam_result$clustering, mean)
  cluster_order <- order(cluster_means)  # Weak → Medium → Strong
  cluster_labels <- c("Strong", "Medium", "Weak")
  cluster_map <- setNames(cluster_labels, cluster_order)
  position_cartesian_filtered$ClusterLabel <- cluster_map[as.character(position_cartesian_filtered$Cluster)]

  # 5. Create improved 3D scatter plot with fixed colors
  p3d <- plot_ly(
    data = position_cartesian_filtered,
    x = ~x, y = ~y, z = ~z,
    type = "scatter3d",
    mode = "markers+text",
    color = ~ClusterLabel,
    ...
    layout(
      ...
      title = paste("Microphone Clustering by IC", component_index),
      legend = list(
        title = list(text = "Cluster Strength"),
        itemsizing = "constant"
      )
    )
  
  print(p3d)  # Ensure the plot is displayed
}
