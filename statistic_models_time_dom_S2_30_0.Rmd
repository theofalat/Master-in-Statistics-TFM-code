...
read_h5_mic_file <- function(filename="S2_30_0.h5", downsample_factor=10){
...
gdpc_model <- gdpc(multivar_data, k = 100)  # k = number of lags
...
# Plot PCA1 separately
ggplot(pca_scores, aes(x = Time, y = PC1)) +
  geom_line(color = "steelblue", size = 1) +
  labs(title = "PCA Component 1 Over Time", y = "PC1 Value", x = "Time") +
  theme_minimal()
...
mic_time_data_new <- mic_time_data[-50, 10000:20000]
...
```{r}
reconstructed <- ica_result$S %*% t(ica_result$A)  # t(A) if A is microphones x components
reconstructed <- t(reconstructed)  # To match original mic x time format
